...
assert.ok(!packageJson.dependencies['@neondatabase/auth'], 'Neon Auth dependency is removed')
...
assert.doesNotMatch(envExample, /NEON_AUTH/, 'Neon Auth env vars are no longer documented')
...
assert.ok(existsSync(join(root, 'src/lib/local-profile.ts')), 'Local profile module exists')
